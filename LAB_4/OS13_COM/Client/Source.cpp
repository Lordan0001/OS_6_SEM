...
int main(int argc, char* argv[])
{
	...
	wchar_t* fileName;
	HT::HTHANDLE* HT;
	//D:\univ\6 sem\os\labs\OS13\HT
	const wchar_t* directoryPath = L"D:\\University\\Current\\OSSP\\All\\Check\\OS13\\HT\\";
	std::wstring filePath(L"D:\\University\\Current\\OSSP\\All\\Check\\OS13\\HT\\test.ht");
	...
	if (HTAPI::OpenConnection()) {
		...
		if (HT != NULL) {
			printf("\nHT-Storage Created filename=%s, snapshotinterval=%d, capacity = %d, maxkeylength = %d, maxdatalength = %d", filePath.c_str(), snapshotIntervalSec, capacity, maxKeyLength, maxPayloadLength);
		}
		else {
			printf("\n some error fail");
		}
